...
export const HeaderItem = ({ text, href }: { text: string, href: string }) => {
    ...
    return (
        <div className={"relative flex items-center justify-center"}>
            ...
            <menu 
                className={`absolute top-6 rounded-lg bg-white z-50 border border-gray7 shadow-lg p-3 w-72 h-96 origin-top transition-all transform-gpu ${visible ? "opacity-100 pointer-events-auto scale-100" : "opacity-0 pointer-events-none scale-95 transition-none"}`}
            >
                hello
            </menu>
        </div>
