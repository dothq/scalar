...
router.get("/api/downloads", async (req, res) => {
    ...
    if(version === "latest") {
        const { data } = await axios.get("https://raw.githubusercontent.com/dothq/browser-ff/main/package.json")
    ...
    const artifactName = `dot-${version}.${language}.${os}-x86${architecture === 64 ? '_64' : ''}.${fileType}`
